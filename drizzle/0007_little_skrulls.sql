CREATE TABLE `pos_fiscal_invoices` (
	...
	CONSTRAINT `pos_fiscal_invoices_id` PRIMARY KEY(`id`),
	CONSTRAINT `pos_fiscal_invoices_sale_unique` UNIQUE(`sale_id`),
	CONSTRAINT `pos_fiscal_invoices_number_unique` UNIQUE(`invoice_number`)
);
...
CREATE TABLE `pos_fiscal_profiles` (
	...
	CONSTRAINT `pos_fiscal_profiles_id` PRIMARY KEY(`id`),
	CONSTRAINT `pos_fiscal_profiles_tax_id_unique` UNIQUE(`tax_id`)
);
--> statement-breakpoint
CREATE TABLE `pos_fiscal_records` (
	`id` int AUTO_INCREMENT NOT NULL,
	...
	CONSTRAINT `pos_fiscal_records_id` PRIMARY KEY(`id`),
	CONSTRAINT `pos_fiscal_records_invoice_unique` UNIQUE(`fiscal_invoice_id`,`record_type`),
	CONSTRAINT `pos_fiscal_records_chain_position_unique` UNIQUE(`chain_position`)
);
--> statement-breakpoint
CREATE TABLE `pos_fiscal_series` (
	`id` int AUTO_INCREMENT NOT NULL,
	`profile_id` int NOT NULL,
	...
	`created_at` timestamp NOT NULL DEFAULT (now()),
	`updated_at` timestamp NOT NULL DEFAULT (now()) ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `pos_fiscal_series_id` PRIMARY KEY(`id`),
	CONSTRAINT `pos_fiscal_series_profile_code_unique` UNIQUE(`profile_id`,`code`)
);
--> statement-breakpoint
CREATE INDEX `pos_fiscal_invoices_profile_index` ON `pos_fiscal_invoices` (`profile_id`,`issued_at`);--> statement-breakpoint
CREATE INDEX `pos_fiscal_records_submission_index` ON `pos_fiscal_records` (`submission_status`,`generated_at`);