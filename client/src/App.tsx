...
type Category = { id: number; name: string; color: string; imageUrl: string | null; iconName: string; sortOrder: number; isFeatured: boolean; isPromotion?: boolean; isActive?: boolean };
...
type CartLine = Product & { quantity: number; unitPriceOverride?: number; discountPercent?: number; pricingMode?: "normal" | "discount" | "cost" | "free" | "promotion"; promotionId?: number | null; promotionSelections?: number[] };
...
function TicketLineEditor({ line, onClose, onSave }: { line: CartLine; onClose: () => void; onSave: (changes: Pick<CartLine, "quantity" | "unitPriceOverride" | "discountPercent" | "pricingMode">) => void }) {
  ...
  return <div className="modal-backdrop ticket-line-editor-backdrop"><section className="ticket-line-editor" role="dialog" aria-modal="true" aria-labelledby="ticket-line-editor-title"><div className="dialog-header"><div><span className="eyebrow">EDITAR LÍNEA</span><h2 id="ticket-line-editor-title">{line.name}</h2></div><button className="post-sale-x-button" onClick={onClose} aria-label="Cerrar"><X size={25} /></button></div><div className="ticket-line-editor__fields"><label>Cantidad<div className="line-quantity-editor"><button type="button" onClick={() => setQuantityValue(String(Math.max(1, Math.floor(Number(quantityValue) || 1) - 1)))} aria-label="Reducir unidades"><Minus size={22} /></button><input type="number" min="1" step="1" inputMode="numeric" value={quantityValue} onChange={(event) => setQuantityValue(event.target.value.replace(/[^0-9]/g, ""))} /><button type="button" onClick={() => setQuantityValue(String(Math.max(1, Math.floor(Number(quantityValue) || 1) + 1)))} aria-label="Aumentar unidades"><Plus size={22} /></button></div></label><label>Precio base<input inputMode="decimal" value={priceValue} onChange={(event) => { setPriceValue(event.target.value); setDiscountValue(0); }} /></label></div><div className="ticket-line-editor__current"><span>Precio de coste: <strong>{euro.format(baseCost)}</strong></span><span>Precio aplicado: <strong>{euro.format(cartLineUnitPrice(line))}</strong></span></div><div className="ticket-line-editor__discounts"><button type="button" className={discountValue === 0 && Number(priceValue.replace(",", ".")) === Number(line.salePrice) ? "discount-option discount-option--selected" : "discount-option"} onClick={() => { setPriceValue(Number(line.salePrice).toFixed(2)); setDiscountValue(0); }}><strong>Precio normal</strong><small>{euro.format(Number(line.salePrice))}</small></button><button type="button" className={discountValue === 10 ? "discount-option discount-option--selected" : "discount-option"} onClick={() => { setPriceValue(Number(line.salePrice).toFixed(2)); setDiscountValue(10); }}><strong>-10%</strong><small>{euro.format(Number(line.salePrice) * .9)}</small></button><button type="button" className={discountValue === 25 ? "discount-option discount-option--selected" : "discount-option"} onClick={() => { setPriceValue(Number(line.salePrice).toFixed(2)); setDiscountValue(25); }}><strong>-25%</strong><small>{euro.format(Number(line.salePrice) * .75)}</small></button><button type="button" className={discountValue === 0 && Number(priceValue.replace(",", ".")) === baseCost ? "discount-option discount-option--selected" : "discount-option"} onClick={() => { setPriceValue(baseCost.toFixed(2)); setDiscountValue(0); }}><strong>Precio de coste</strong><small>{euro.format(baseCost)}</small></button><button type="button" className={discountValue === 100 ? "discount-option discount-option--selected" : "discount-option"} onClick={() => { setPriceValue(Number(line.salePrice).toFixed(2)); setDiscountValue(100); }}><strong>100% descuento</strong><small>0,00 €</small></button></div><p className="helper-text">El descuento se aplica a esta línea y el stock se descuenta normalmente.</p><div className="ticket-line-editor__actions"><button className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" onClick={() => save(discountValue === 0 && Number(priceValue.replace(",", ".")) === Number(line.salePrice) ? "normal" : discountValue === 100 ? "free" : discountValue === 0 && Number(priceValue.replace(",", ".")) === baseCost ? "cost" : "discount", discountValue)}><Check size={17} /> Aplicar</button></div></section></div>;
}

function PromotionSelector({ product, promotion, onClose, onConfirm }: { product: Product; promotion: Promotion; onClose: () => void; onConfirm: (selections: number[]) => void }) {
  const [selections, setSelections] = useState<Record<number, number>>({});
  const ready = promotion.slots.every((slot) => selections[slot.id]);
  return <div className="modal-backdrop promotion-selector-backdrop"><section className="promotion-selector" role="dialog" aria-modal="true" aria-labelledby="promotion-selector-title"><div className="dialog-header"><div><span className="eyebrow">PROMOCIÓN</span><h2 id="promotion-selector-title">{promotion.name}</h2><p>{euro.format(Number(promotion.comboPrice))} · Elige una opción de cada familia</p></div><button className="post-sale-x-button" onClick={onClose} aria-label="Cerrar"><X size={25} /></button></div><div className="promotion-selector__choices">{promotion.slots.map((slot, index) => <section className="promotion-choice-group" key={slot.id}><div><span className="eyebrow">OPCIÓN {index + 1}</span><h3>{slot.label}</h3></div><div className="promotion-choice-grid">{slot.products.map((choice) => <button className={selections[slot.id] === choice.productId ? "promotion-choice promotion-choice--selected" : "promotion-choice"} key={choice.productId} onClick={() => setSelections((current) => ({ ...current, [slot.id]: choice.productId }))}><span>{choice.productName}</span>{selections[slot.id] === choice.productId && <Check size={18} />}</button>)}</div></section>)}</div><p className="helper-text">Se descontará una unidad del stock de cada artículo seleccionado.</p><div className="promotion-selector__footer"><button className="secondary-button" onClick={onClose}>Cancelar</button><button className="primary-button" disabled={!ready} onClick={() => onConfirm(promotion.slots.map((slot) => selections[slot.id]))}>Añadir combo · {euro.format(Number(promotion.comboPrice))}</button></div></section></div>;
}
...
function PosScreen({ onOpenMenu }: { onOpenMenu: () => void }) {
  ...
  const addPromotionToCart = (selections: number[]) => {
    if (!promotionChoice) return;
    const { product, promotion } = promotionChoice;
    setCart((current) => {
      const existing = current.find((line) => line.id === product.id && line.promotionId === promotion.id && JSON.stringify(line.promotionSelections ?? []) === JSON.stringify(selections));
      if (existing) return current.map((line) => line === existing ? { ...line, quantity: line.quantity + 1 } : line);
      return [...current, { ...product, quantity: 1, promotionId: promotion.id, promotionSelections: selections, pricingMode: "promotion" }];
    });
  ...
  const updateCartLine = (lineKey: string, changes: Pick<CartLine, "quantity" | "unitPriceOverride" | "discountPercent" | "pricingMode">) => {
    setCart((current) => current.map((line) => cartLineIdentity(line) === lineKey ? { ...line, ...changes } : line));
...
type ReportsData = {
  ...
  series: Array<{ label: string; total: string; tickets: number; cash: string; card: string }>;
...
type LoyverseCatalogRow = { id: string; name: string; category: string; imageUrl: string | null; sku: string | null; barcode: string | null; variants: number; price: number | null; stock: number; availableForSale: boolean; deleted: boolean; updatedAt: string | null };
...
function AdminScreen({ onBack, onOpenMenu }: { onBack: () => void; onOpenMenu: () => void }) {
  ...
  const [categoryForm, setCategoryForm] = useState({ name: "", color: "#4C8A5A", iconName: "Folder", isPromotion: false });
  ...
  const [productSort, setProductSort] = useState<"name" | "price" | "stock" | "sold">("name");
  ...
  const reportsQuery = useQuery({
    queryKey: ["reports", reportPeriod, reportFrom, reportTo, reportSource],
    queryFn: () => api<ReportsData>(`/admin/reports?period=${reportPeriod}&source=${reportSource}${reportPeriod === "custom" && reportFrom ? `&from=${reportFrom}` : ""}${reportPeriod === "custom" && reportTo ? `&to=${reportTo}` : ""}`),
  });
  ...
  const syncLoyverseCatalogMutation = useMutation({
    mutationFn: () => api<{ success: boolean; items: number; categories: number; inventoryLevels: number }>("/admin/loyverse/sync/catalog", { method: "POST" }),
    onSuccess: (result) => { toast.success("Catálogo de Loyverse sincronizado", { description: `${result.items} artículos · ${result.categories} familias · ${result.inventoryLevels} niveles de stock` }); queryClient.invalidateQueries({ queryKey: ["loyverse-status"] }); queryClient.invalidateQueries({ queryKey: ["loyverse-dashboard"] }); },
  ...
  const importLoyverseCatalogMutation = useMutation({
    mutationFn: () => api<{ success: boolean; categoriesCreated: number; categoriesUpdated: number; productsCreated: number; productsUpdated: number; stockUpdated: number; skipped: number; skippedDetails: string[] }>("/admin/loyverse/import/catalog", { method: "POST", body: JSON.stringify({ storeId: loyverseStoreId || undefined }) }),
    onSuccess: (result) => { toast.success("Catálogo importado al TPV", { description: `${result.productsCreated} nuevos · ${result.productsUpdated} actualizados · ${result.stockUpdated} stocks sincronizados${result.skipped ? ` · ${result.skipped} omitidos por conflicto` : ""}` }); queryClient.invalidateQueries({ queryKey: ["admin-products"] }); queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); queryClient.invalidateQueries({ queryKey: ["admin-inventory"] }); queryClient.invalidateQueries({ queryKey: ["catalog"] }); queryClient.invalidateQueries({ queryKey: ["loyverse-dashboard"] }); },
  ...
  const createCategoryMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Familia creada"); setCategoryForm({ name: "", color: "#4C8A5A", iconName: "Folder", isPromotion: false }); setCategoryImageUrl(null); setCategoryFormOpen(false); queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); queryClient.invalidateQueries({ queryKey: ["categories"] }); },
  ...
  const createPromotionMutation = useMutation({
    mutationFn: (input: { productId: number; name: string; comboPrice: number; slots: Array<{ label: string; categoryId: number; productIds: number[] }> }) => api<{ id: number }>("/admin/promotions", { method: "POST", body: JSON.stringify(input) }),
    onSuccess: () => { toast.success("Promoción guardada"); setPromotionFormOpen(false); setPromotionForm({ productId: "", name: "", comboPrice: "", slots: [{ label: "", categoryId: "", productIds: [] }] }); invalidateAdmin(); },
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {tab === "loyverse" && <LoyversePanel status={loyverseStatusQuery.data} settings={settingsQuery.data} dashboard={loyverseDashboardQuery.data} loading={loyverseDashboardQuery.isLoading || loyverseStatusQuery.isLoading} configForm={loyverseConfigForm} setConfigForm={setLoyverseConfigForm} savingConfig={saveLoyverseSettingsMutation.isPending} onSaveConfig={() => saveLoyverseSettingsMutation.mutate()} testingConnection={testLoyverseMutation.isPending} onTestConnection={() => testLoyverseMutation.mutate()} importingCatalog={importLoyverseCatalogMutation.isPending} onImportCatalog={() => { if (window.confirm("Se actualizarán las familias, artículos, precios, imágenes y stock del TPV usando la caché de Loyverse. No se modificarán ventas ni caja. ¿Continuar?")) importLoyverseCatalogMutation.mutate(); }} storeId={loyverseStoreId} setStoreId={setLoyverseStoreId} from={loyverseFrom} setFrom={setLoyverseFrom} to={loyverseTo} setTo={setLoyverseTo} syncingCatalog={syncLoyverseCatalogMutation.isPending} syncingSales={syncLoyverseSalesMutation.isPending} syncingAll={syncLoyverseAllMutation.isPending} onSyncCatalog={() => syncLoyverseCatalogMutation.mutate()} onSyncSales={() => syncLoyverseSalesMutation.mutate()} onSyncAll={() => syncLoyverseAllMutation.mutate()} />}
...
function ReportsPanel({ data, period, setPeriod, from, setFrom, to, setTo, source, setSource, isLoading }: { data?: ReportsData; period: string; setPeriod: (value: string) => void; from: string; setFrom: (value: string) => void; to: string; setTo: (value: string) => void; source: "all" | "loyverse" | "local"; setSource: (value: "all" | "loyverse" | "local") => void; isLoading: boolean }) {
  ...
  return <div className="admin-page reports-page">
    ...
    {isLoading ? <div className="admin-card admin-empty">Calculando informe…</div> : <>
      ...
      <section className="admin-card report-chart-card"><div className="admin-card__header"><div><span className="eyebrow">EVOLUCIÓN</span><h2>Ventas por periodo</h2></div><span className="chart-caption">Importe vendido</span></div>{series.length === 0 ? <div className="admin-empty">No hay ventas en el periodo seleccionado.</div> : <div className="report-bars">{series.map((item) => <div className="report-bar-column" key={item.label} title={`${item.label}: ${euro.format(Number(item.total))}`}><strong>{euro.format(Number(item.total))}</strong><div className="report-bar-track"><div className="report-bar-fill" style={{ height: `${Math.max(5, (Number(item.total) / maxSeries) * 100)}%` }} /></div><small>{item.label}</small><span>{item.tickets} tickets</span></div>)}</div>}</section>
...
function LoyversePanel({ status, settings, dashboard, loading, configForm, setConfigForm, savingConfig, onSaveConfig, testingConnection, onTestConnection, importingCatalog, onImportCatalog, storeId, setStoreId, from, setFrom, to, setTo, syncingCatalog, syncingSales, syncingAll, onSyncCatalog, onSyncSales, onSyncAll }: { status?: LoyverseStatus; settings?: LoyverseSettings; dashboard?: LoyverseDashboard; loading: boolean; configForm: LoyverseConfigForm; setConfigForm: (value: LoyverseConfigForm) => void; savingConfig: boolean; onSaveConfig: () => void; testingConnection: boolean; onTestConnection: () => void; importingCatalog: boolean; onImportCatalog: () => void; storeId: string; setStoreId: (value: string) => void; from: string; setFrom: (value: string) => void; to: string; setTo: (value: string) => void; syncingCatalog: boolean; syncingSales: boolean; syncingAll: boolean; onSyncCatalog: () => void; onSyncSales: () => void; onSyncAll: () => void }) {
