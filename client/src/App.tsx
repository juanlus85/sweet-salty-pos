...
type AdminTab = "analysis" | "reports" | "fiscal" | "overview" | "categories" | "products" | "inventory" | "suppliers" | "purchases" | "sales" | "cash" | "settings" | "loyverse";
...
function AdminScreen({ onBack }: { onBack: () => void }) {
  ...
  const reportsQuery = useQuery({
    queryKey: ["reports", reportPeriod, reportFrom, reportTo],
    queryFn: () => api<ReportsData>(`/admin/reports?period=${reportPeriod}${reportPeriod === "custom" && reportFrom ? `&from=${reportFrom}` : ""}${reportPeriod === "custom" && reportTo ? `&to=${reportTo}` : ""}`),
  });
  ...
  const syncLoyverseAllMutation = useMutation({
    mutationFn: async () => {
      ...
      const sales = await api<{ success: boolean; receipts: number; shifts: number }>("/admin/loyverse/sync/sales", { method: "POST", body: JSON.stringify({ from: loyverseFrom ? new Date(`${loyverseFrom}T00:00:00`).toISOString() : undefined, to: loyverseTo ? new Date(`${loyverseTo}T23:59:59.999`).toISOString() : undefined }) });
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {tab === "reports" && <ReportsPanel data={reportsQuery.data} period={reportPeriod} setPeriod={setReportPeriod} from={reportFrom} setFrom={setReportFrom} to={reportTo} setTo={setReportTo} isLoading={reportsQuery.isLoading} />}
...
function ReportsPanel({ data, period, setPeriod, from, setFrom, to, setTo, isLoading }: { data?: ReportsData; period: string; setPeriod: (value: string) => void; from: string; setFrom: (value: string) => void; to: string; setTo: (value: string) => void; isLoading: boolean }) {
...
function LoyversePanel({ status, settings, dashboard, loading, configForm, setConfigForm, savingConfig, onSaveConfig, testingConnection, onTestConnection, importingCatalog, onImportCatalog, storeId, setStoreId, from, setFrom, to, setTo, syncingCatalog, syncingAll, onSyncCatalog, onSyncAll }: { status?: LoyverseStatus; settings?: LoyverseSettings; dashboard?: LoyverseDashboard; loading: boolean; configForm: LoyverseConfigForm; setConfigForm: (value: LoyverseConfigForm) => void; savingConfig: boolean; onSaveConfig: () => void; testingConnection: boolean; onTestConnection: () => void; importingCatalog: boolean; onImportCatalog: () => void; storeId: string; setStoreId: (value: string) => void; from: string; setFrom: (value: string) => void; to: string; setTo: (value: string) => void; syncingCatalog: boolean; syncingAll: boolean; onSyncCatalog: () => void; onSyncAll: () => void }) {
