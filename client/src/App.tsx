...
function AdminScreen({ onBack }: { onBack: () => void }) {
  ...
  const filteredAdminProducts = useMemo(() => {
    ...
    const rows = (productsQuery.data ?? []).filter((product) => productCategoryFilter === "all" || String(product.categoryId) === productCategoryFilter).filter((product) => {
      const stock = Number(product.stock);
  ...
  }, [productsQuery.data, reportQuery.data, productCategoryFilter, productStockFilter, productSort]);
