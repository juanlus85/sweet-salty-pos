...
function AdminScreen({ onBack }: { onBack: () => void }) {
  ...
  const saveLoyverseSettingsMutation = useMutation({
    ...
    onSuccess: (settings) => { setLoyverseConfigForm((current) => ({ ...current, apiToken: "", clearToken: false })); queryClient.setQueryData(["admin-settings"], { ...settingsQuery.data, ...settings }); queryClient.invalidateQueries({ queryKey: ["loyverse-status"] }); queryClient.invalidateQueries({ queryKey: ["loyverse-dashboard"] }); toast.success("Configuración de Loyverse guardada"); },
  ...
  const testLoyverseMutation = useMutation({
    ...
    onSuccess: (result) => { toast.success("Conexión con Loyverse correcta", { description: result.merchantName ? `Comercio: ${result.merchantName}` : result.apiBase }); queryClient.invalidateQueries({ queryKey: ["loyverse-status"] }); },
  ...
  const syncLoyverseAllMutation = useMutation({
    mutationFn: () => api<{ success: boolean; receipts: number; shifts: number }>("/admin/loyverse/sync", { method: "POST", body: JSON.stringify({ from: loyverseFrom ? new Date(`${loyverseFrom}T00:00:00`).toISOString() : undefined, to: loyverseTo ? new Date(`${loyverseTo}T23:59:59.999`).toISOString() : undefined }) }),
    onSuccess: (result) => { toast.success("Loyverse sincronizado", { description: `${result.receipts} recibos y ${result.shifts} turnos importados. Solo lectura.` }); queryClient.invalidateQueries({ queryKey: ["loyverse-status"] }); queryClient.invalidateQueries({ queryKey: ["loyverse-dashboard"] }); },
  ...
  return (
    <main className="admin-shell">
      ...
      <section className="admin-content">
        ...
        {tab === "loyverse" && <LoyversePanel status={loyverseStatusQuery.data} settings={settingsQuery.data} dashboard={loyverseDashboardQuery.data} loading={loyverseDashboardQuery.isLoading || loyverseStatusQuery.isLoading} configForm={loyverseConfigForm} setConfigForm={setLoyverseConfigForm} savingConfig={saveLoyverseSettingsMutation.isPending} onSaveConfig={() => saveLoyverseSettingsMutation.mutate()} testingConnection={testLoyverseMutation.isPending} onTestConnection={() => testLoyverseMutation.mutate()} storeId={loyverseStoreId} setStoreId={setLoyverseStoreId} from={loyverseFrom} setFrom={setLoyverseFrom} to={loyverseTo} setTo={setLoyverseTo} syncingCatalog={syncLoyverseCatalogMutation.isPending} syncingAll={syncLoyverseAllMutation.isPending} onSyncCatalog={() => syncLoyverseCatalogMutation.mutate()} onSyncAll={() => syncLoyverseAllMutation.mutate()} />}
