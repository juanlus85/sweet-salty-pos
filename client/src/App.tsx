...
type AdminTab = "analysis" | "reports" | "overview" | "categories" | "products" | "inventory" | "suppliers" | "purchases" | "sales" | "cash" | "settings";
...
function ReportsPanel({ data, period, setPeriod, from, setFrom, to, setTo, isLoading }: { data?: ReportsData; period: string; setPeriod: (value: string) => void; from: string; setFrom: (value: string) => void; to: string; setTo: (value: string) => void; isLoading: boolean }) {
  ...
  return <div className="admin-page reports-page">
    ...
    {isLoading ? <div className="admin-card admin-empty">Calculando informe…</div> : <>
      ...
      <section className="admin-card"><div className="admin-card__header"><div><span className="eyebrow">FISCALIDAD</span><h2>Desglose de IVA</h2></div><span className="chart-caption">Según líneas vendidas</span></div><div className="data-table-wrap"><table className="data-table report-table"><thead><tr><th>Tipo</th><th>Unidades</th><th>Base/ventas</th><th>IVA</th></tr></thead><tbody>{data?.vatBreakdown.map((item) => <tr key={item.vat}><td><strong>{item.vat}%</strong></td><td>{item.units}</td><td className="table-money">{euro.format(Number(item.revenue))}</td><td className="table-money">{euro.format(Number(item.vatAmount))}</td></tr>)}</tbody></table>{!data?.vatBreakdown.length && <div className="admin-empty">No hay desglose fiscal.</div>}</div></section>
    </>}
  </div>;
}
