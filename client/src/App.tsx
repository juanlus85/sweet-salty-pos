...
function CheckoutDialog({ cart, total, processing, onClose, onComplete }: { cart: CartLine[]; total: number; processing: boolean; onClose: () => void; onComplete: (method: "cash" | "card", tendered?: number, reference?: string) => void }) {
  ...
  const completeCash = (amount?: number) => { if (!processing) onComplete("cash", amount); };
  const completeCard = () => { if (!processing) onComplete("card"); };
  return (
    <div className="checkout-screen" role="dialog" aria-modal="true" aria-labelledby="payment-title">
      <aside className="checkout-ticket"><header><h2>Ticket</h2><button type="button" className="icon-button" onClick={onClose} aria-label="Volver al ticket"><ArrowLeft size={20} /></button></header><div className="checkout-ticket__lines">{cart.map((line) => <div key={cartLineIdentity(line)}><span>{line.name} <small>x {line.quantity}</small>{line.promotionComponents?.length ? <small className="checkout-ticket__components">Incluye: {line.promotionComponents.join(" · ")}</small> : null}</span><strong>{euro.format(cartLineTotal(line))}</strong></div>)}</div><div className="checkout-ticket__tax">Impuesto (incluido)</div><div className="checkout-ticket__total"><strong>Total</strong><strong>{euro.format(total)}</strong></div></aside>
      <section className="checkout-payment">
        <header className="checkout-payment__header"><button type="button" className="checkout-back" onClick={onClose}><ArrowLeft size={24} /></button><span>PAGO</span></header>
        <div className="checkout-payment__body"><h1 id="payment-title">{euro.format(total)}</h1><p>Cantidad total a pagar</p><div className="checkout-tabs"><button type="button" className={method === "cash" ? "checkout-tab checkout-tab--active" : "checkout-tab"} onClick={() => setMethod("cash")}><Banknote size={18} /> Efectivo</button><button type="button" className="checkout-tab checkout-tab--direct" disabled={processing} onClick={completeCard} aria-label="Cobrar directamente con tarjeta"><CreditCard size={25} /> <span>{processing ? "Registrando…" : "Tarjeta"}</span></button></div>{method === "cash" ? <><label className="checkout-money-field"><span>Efectivo recibido <small>Opcional · vacío = importe exacto</small></span><div><Banknote size={22} /><input inputMode="decimal" value={tendered} onChange={(event) => setTendered(event.target.value)} autoFocus placeholder={total.toFixed(2)} /><b>€</b></div></label><div className="quick-cash" aria-label="Importes rápidos">{quickAmounts.map((amount) => <button key={amount} type="button" disabled={processing} onClick={() => completeCash(amount)}><span className={amount <= 2 ? "cash-denomination cash-denomination--coin" : "cash-denomination cash-denomination--bill"}>{amount} €</span><strong>{euro.format(amount)}</strong></button>)}</div><div className="checkout-change"><span>Cambio</span><strong>{euro.format(change)}</strong></div><p className="checkout-cash-hint">Si no indicas cuánto entrega el cliente, se registra el importe exacto del ticket.</p><button type="button" className="pay-confirm-button" disabled={!canCompleteCash || processing} onClick={() => completeCash(tendered.trim().length ? tenderedValue : undefined)}><Banknote size={21} /> {processing ? "Registrando…" : "Cobrar en efectivo"}</button></> : <button type="button" className="card-fast-pay" disabled={processing} onClick={completeCard}><CreditCard size={58} /><strong>{processing ? "REGISTRANDO…" : "PAGO CON TARJETA"}</strong><span>Confirma el pago en el datáfono y pulsa aquí para registrarlo</span></button>}</div>
      </section>
    </div>
  );
}
...
function PosScreen({ onOpenMenu }: { onOpenMenu: () => void }) {
  ...
  return (
    <main className="pos-shell">
      ...
      {isPaying && <CheckoutDialog cart={cart} total={cartTotal} processing={checkoutMutation.isPending} onClose={() => !checkoutMutation.isPending && setIsPaying(false)} onComplete={(method, tendered, reference) => checkoutMutation.mutate({ method, tendered, reference })} />}
