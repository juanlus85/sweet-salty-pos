...
function AdminScreen({ onBack }: { onBack: () => void }) {
  ...
  const importLoyverseCatalogMutation = useMutation({
    mutationFn: () => api<{ success: boolean; categoriesCreated: number; categoriesUpdated: number; productsCreated: number; productsUpdated: number; stockUpdated: number; skipped: number }>("/admin/loyverse/import/catalog", { method: "POST", body: JSON.stringify({ storeId: loyverseStoreId || undefined }) }),
    onSuccess: (result) => { toast.success("Catálogo importado al TPV", { description: `${result.productsCreated} nuevos · ${result.productsUpdated} actualizados · ${result.stockUpdated} stocks sincronizados` }); queryClient.invalidateQueries({ queryKey: ["admin-products"] }); queryClient.invalidateQueries({ queryKey: ["admin-categories"] }); queryClient.invalidateQueries({ queryKey: ["admin-inventory"] }); queryClient.invalidateQueries({ queryKey: ["catalog"] }); queryClient.invalidateQueries({ queryKey: ["loyverse-dashboard"] }); },
